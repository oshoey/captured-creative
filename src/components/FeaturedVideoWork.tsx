...
const YOUTUBE_ID = '_4sSa749kJY'
const YOUTUBE_EMBED = `https://www.youtube.com/embed/${YOUTUBE_ID}?autoplay=1&rel=0&modestbranding=1&iv_load_policy=3&controls=1`
...
export default function FeaturedVideoWork() {
  ...
  const openModal = useCallback(() => {
    setModalOpen(true)
    setIframeSrc(YOUTUBE_EMBED)
  }, [])

  const closeModal = useCallback(() => {
    // Clear src first — stops YouTube audio immediately during the exit animation
    setIframeSrc('')
    setModalOpen(false)
  }, [])
    ...
    <AnimatePresence>
      {modalOpen && (
        ...
        >
          ...
          {/* YouTube embed — 16:9 container, stopPropagation prevents backdrop close */}
          <motion.div
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div>
            <p className="label" style={{ color: '#A1A1A1', marginBottom: '20px', letterSpacing: '0.22em' }}>
              Featured Video Work
            </p>
            <h2 style={{
          ...
          <p className="label" style={{ color: 'rgba(161,161,161,0.45)', flexShrink: 0, textAlign: 'right' }}>
            Cinematic
          </p>
        </motion.div>
        ...
        >
          ...
          >
            {/* Looping background preview — muted, local */}
            <motion.video
              ref={bgVideoRef}
            ...
            <motion.div
              ...
              style={{ position: 'absolute', inset: 0, background: 'rgba(5,5,5,0.42)', pointerEvents: 'none' }}
            />

            {/* Year badge */}
            <div style={{ position: 'absolute', top: '32px', right: '36px' }}>
              <span className="label" style={{ color: 'rgba(245,245,245,0.28)', fontSize: '10px', letterSpacing: '0.22em' }}>
                2024
              </span>
            </div>
            ...
            <div style={{
              ...
              <motion.div
                animate={{ opacity: hovered ? 1 : 0.35, x: hovered ? 5 : 0 }}
                transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
                style={{ flexShrink: 0, paddingBottom: '8px' }}
              >
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none"
                  stroke="rgba(245,245,245,0.82)" strokeWidth="1.2"
